...
- 候选已能生成 prompt_rule、tool_schema_hint 和 skill 第一版审查项；接受后会写入本地 accepted guidance store，并能按相关性进入 provider context。
- 尚未形成“接受候选后再次跑 benchmark 证明收益”的闭环。

近期任务：

- 完善 `data/analysis-reports/*.json -> candidate -> TUI review -> accepted runtime context` 的真实 TUI 场景覆盖。
- 扩展 tool_schema_hint 的 ToolPool / tool_search 使用路径，让 accepted hint 不只作为 provider context 存在。
...
1. Analysis Report -> Evolution Candidate
   ...
   - 已完成第一版：prompt_rule / skill / tool_schema_hint 接受后可被 runtime context 召回。
   - 下一步：补 TUI review flow 和 benchmark proof。
