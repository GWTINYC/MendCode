...
截至 2026-04-22，项目已经跨过“基础骨架是否能跑”的阶段，进入“如何最短路径做出最小修复 Agent”的阶段。
...
- 工具调用输入 / 输出在 runner 中落地
- 工具级 trace 事件
- 更完整的 run state 步骤推进
- 固定 action budget 的最小 loop
- 至少 1 条真实 demo 任务走通 `read -> search -> patch -> verify`

验收：

- `task run` 不再只做 verification，还能在固定流程中调用工具
- trace 可以回放每一步工具调用与结果
- 至少在窄范围 demo 任务上完成一次真实修复尝试

### Phase 3：补最小上下文工程

目标：只围绕真实闭环暴露的瓶颈补上下文能力，而不是先搭一个大平台。
...
- 同一 demo 任务下无效读文件明显减少
- 平均步骤数下降或定位稳定性提升
- 不引入重型索引或过早缓存体系

### Phase 4：补评测闭环

目标：把系统从“能跑 demo”提升到“每次改动都可比较”。
...
- 第 4 周：完成 Phase 2
- 第 5 周：完成 Phase 3 到 Phase 4 的最小子集
- 第 6 周：完成 Phase 5 的最小子集

如果资源更紧，至少也要保证：

- 第一个“真实读搜改验”演示版本必须尽快出现
- 第一个可批量评测版本必须在可演示版本之后尽快出现
...
- 之后依次推进：
  - 先把最小工具调用闭环接进 runner
  - 再用真实 demo 暴露的瓶颈倒逼上下文工程
  - 最后再补评测闭环，而不是先做大而全的 context 平台
...
- 当前下一阶段的重点已经转为“工具层最小闭环”：
  ...
  - 下一刀直接把 orchestrator 接到最小工具调用链
  - 上下文工程只围绕这个闭环的真实瓶颈补，不提前整包展开
...
截至当前，这个判断已经被进一步验证：项目不再只停留在 Phase 0/1 的运行骨架，workspace 隔离、受控 verification 和只读工具第一刀都已落地。接下来的关键，不是继续扩散能力面，而是按顺序补完 `apply_patch` 与最小工具调用闭环，让仓库从“能受控执行”升级为“能在受控工作区内完成最小读写闭环”。

---

## 19. 2026-04-22 Phase 2C 进展校准

本轮开发已经完成“先收口 command policy，再收口 runner 执行契约”的第一刀，重点不是新增能力，而是让已经存在的安全边界和状态表达真正落到执行链。

### 19.1 本轮已完成

- `runner` 已开始真正执行 `TaskSpec.allowed_tools` 授权约束
- fixed-flow 中未授权工具不会被静默调用，而是返回结构化 `ToolResult(status="rejected")`
- 未授权工具同样会进入现有 `run.tool.started` / `run.tool.completed` trace
- `RunState.current_step` 不再在失败路径上一律写成 `summarize`
- 当前失败阶段已能区分：
  - `bootstrap`
  - `locate`
  - `inspect`
  - `patch`
  - `verify`
- CLI 已能直接展示 unauthorized tool 场景下的失败阶段

### 19.2 这一步的意义

这一步完成后，MendCode 从“fixed-flow 能跑通”进一步升级为“fixed-flow 的执行边界更可信、失败定位更直接”。这对后面的 demo 扩展和 batch eval 都是前置条件，因为如果 runner 自己不执行授权边界、失败阶段又不透明，那么后续指标和样例都不够可信。

### 19.3 下一步

按当前主线，下一步不应继续深挖 runner 细枝末节，而应转向：

- 固化 success / unauthorized / ambiguous-search / verification-fail 的 demo 任务组
- 同步 README 与任务示例，明确当前支持边界
- 再补最小 batch eval，把失败阶段和工具调用次数沉淀成可比较结果

---

## 20. 2026-04-22 Phase 2C demo suite 收口结果

上一节的三项“下一步”现在已经完成前两项，项目状态因此发生了一个明显变化：MendCode 已经不只是“runner 内部 fixed-flow 可跑”，而是第一次把 demo 样例、CLI 覆盖和 README 使用入口一起收口到了同一套任务资产上。

### 20.1 本轮新增完成项

- 已落地稳定的 repo-native demo task suite：
...
- 已补充 `data/demo-fixtures/ambiguous-search/`，让 search 歧义场景不再依赖 README 等易漂移文件
- 已删除旧的 `data/tasks/demo.json` 单 fixture 入口
- `tests/unit/test_task_schema.py` 已切换到新 demo suite
- `tests/integration/test_cli.py` 已覆盖：
  - `task validate`
  - `task show`
  - `task run`
  - README 命令契约
- README 已切换到新的 demo suite 命令路径，并明确四类 demo 的语义

### 20.2 这一步的意义

这一步完成后，MendCode 的“最小修复闭环”第一次具备了可演示、可回归、可文档化的统一样例面：

- demo 任务不再只有一个共享 fixture
- 失败路径不再只存在于内部测试，而是被整理成可直接运行的样例
- CLI、README、任务资产和 trace 语义开始对齐

这比单纯再优化 runner 细节更重要，因为后续 batch eval、trace 报表和对外演示都需要一套稳定样例作为共同基线。

### 20.3 当前阶段判断

截至当前，主线已经完成：

- command policy / executor / worktree manager 收口
- fixed-flow `read -> search -> patch -> verify` 最小闭环
- unauthorized tool / ambiguous search / verification fail 的显式失败表达
- repo-native demo suite、CLI 覆盖和 README 对齐

当前还没有完成、且最值得继续推进的，不是再扩工具细节，而是：

- 最小 batch eval
- trace 聚合与结果对比
- 基于 demo suite 的批量回归能力

### 20.4 下一步优先级

后续应按下面顺序推进，不要回头在 runner 局部继续打磨：

1. 基于当前 demo suite 增加最小 batch eval 入口
2. 先沉淀每条 demo 的通过/失败、失败阶段、工具结果等核心指标
3. 再考虑把这些指标输出成 Markdown / JSON 报表

一句话说，当前已经到了：

`不要再问 runner 能不能再优雅一点，而要开始让 demo 结果可批量比较。`

---

## 21. 2026-04-23 Phase 2C MVP eval 落地结果

上一轮“补最小 batch eval”的目标已经推进到可运行状态。当前 MendCode 不再只是单条 demo 可跑，而是具备了最小批量评测入口和第一条真实 Python 代码修复 demo。

### 21.1 本轮已完成

- 新增 `Settings.evals_dir`，评测运行产物统一落到 `data/evals/`
- 新增 `BatchEvalResult` / `BatchEvalSummary` schema，批量结果可写入 JSON
- 新增 `app/eval/batch.py`，按顺序调用现有 `run_task_preview`
- 新增 CLI：
...
- 新增第五条 demo：
  ...
  - 通过 patch `return a - b` 到 `return a + b`
  - 用 `pytest` 证明真实 Python 单测修复成功
- README 已补充 MVP Eval 使用方式

### 21.2 当前能力边界

当前 eval 仍然是 MVP 形态：

- 它是顺序 batch runner，不做并发
- 它复用 fixed-flow runner，不引入 planner
- 它只统计任务级状态和 summary，不做复杂指标平台
- 它适合做 demo suite 的批量回归，不适合宣称通用自动修复能力

这个边界是合理的。当前最重要的不是继续扩 eval 平台，而是用它固定比较后续策略变化。

### 21.3 下一步

后续开发应优先围绕“让评测结果可比较”推进：

1. 用现有 5 条 demo 固定一组 MVP baseline
2. 只补最小 trace / eval 字段聚合，例如失败阶段、工具结果、验证计数
3. 当 batch eval 能稳定暴露瓶颈后，再决定是否扩 demo 或接入更动态的决策 loop

一句话更新当前主线：

`batch eval 已经落地，下一步不是继续造平台，而是用它比较每次策略变化是否真的变好。`

---

## 22. 2026-04-23 全局进展复盘：MVP-0 基线已形成

结合当前代码和最近一组提交，MendCode 已经进入一个新的阶段：它不再只是“模块具备”，而是已经形成了一个可以演示、可以批量运行、可以继续评测的 MVP-0 基线。

### 22.1 当前已经成型的闭环

当前主链路已经覆盖：

- 任务输入：`TaskSpec` 严格 schema 与 repo-native demo task
- 工作区隔离：每次 `task run` 进入独立 `.worktrees/preview-<id>/`
- 工具执行：`read_file` / `search_code` / `apply_patch` 已接入 fixed-flow runner
- 安全边界：`allowed_tools` 已从 schema 声明进入 runner 执行期约束
- 状态表达：失败路径能保留 `bootstrap` / `locate` / `inspect` / `patch` / `verify`
- 验证执行：verification command 在受控工作区内运行并写入 trace
- 结果沉淀：`RunState`、JSONL trace、batch `summary.json` / `summary.md`
- 用户入口：README 已提供单任务 demo 和 MVP batch eval 命令

当前 5 条 demo 的语义也已经比较清楚：

- `success.json`：成功修复 README 文本
- `unauthorized-tool.json`：验证未授权工具被拒绝
- `ambiguous-search.json`：验证 locate 阶段多候选失败
- `verification-fail.json`：验证 verify 阶段失败表达
- `python-unit-fix.json`：验证真实 Python 源码修复并让 pytest 通过

### 22.2 当前能力的真实边界

虽然 MVP-0 基线已形成，但它仍不是“通用自治 Agent”：

- runner 仍是固定流程，不是模型驱动 planner
- batch eval 仍是顺序执行，不做并发和调度
- eval summary 现在统计的是 runner 机械状态，不是“任务是否符合预期结果”
- demo suite 仍偏小，主要用于锁住主链路，不用于证明广泛任务泛化
- trace 已经可回放，但还没有被聚合成稳定的策略指标

这意味着当前不要把项目包装成“已经能自动修所有代码问题”。更准确的表述是：

`MendCode 已经具备一个可控、可观察、可批量运行的本地代码修复最小闭环。`

### 22.3 下一步最短路径

后续最值得做的是把“运行结果”升级成“可判断结果”：

1. 为 demo task 增加最小期望语义，例如 expected status、expected failure step、expected tool status
2. 让 batch eval 输出“是否符合预期”，而不只输出 completed / failed
3. 在 summary 中聚合失败阶段、工具拒绝、验证计数等最小指标
4. 用当前 5 条 demo 建立第一版 baseline，再评估是否需要第 6 条 demo
...
`MVP-0 已经跑起来了，下一步要让 eval 告诉我们“跑得对不对”。`

---

## 23. 2026-04-23 用户可用 MVP Quickstart 收口

从真实用户使用角度复盘后，MVP-0 仍然有一个明显缺口：内部 demo suite 已经能跑，但用户最自然会尝试的 `data/tasks/demo.json` 入口不存在。这会让 `mendcode task validate/show/run data/tasks/demo.json` 三条基础命令直接失败。

本轮已将默认入口恢复为首要用户体验目标。

### 23.1 本轮已完成

- 恢复 `data/tasks/demo.json` 作为默认 quickstart 任务
- 默认任务使用稳定成功路径：
  - 目标文件：`README.md`
  - 工具链：`search_code -> read_file -> apply_patch -> verify`
  - 验证方式：检查 README 中的目标文本是否被替换
- README 增加最短 quickstart：
  - `mendcode version`
  - `mendcode health`
  - `mendcode task validate data/tasks/demo.json`
  - `mendcode task show data/tasks/demo.json`
  - `mendcode task run data/tasks/demo.json`
- 测试已锁住默认 demo 可加载、README 命令存在、默认 demo 可 validate/show/run

### 23.2 当前 MVP 定义更新

当前 MVP 的用户定义应从“开发者能跑 demo suite”更新为：

`用户安装后能复制 README 中 5 条命令，完成一次可观察的本地修复实验。`

这仍然是 fixed-flow MVP，不是通用自治 Agent。但它已经具备最小用户可用性：

- 有默认任务
- 有健康检查
- 有任务预览
- 有任务运行
- 有验证结果、trace_path 和 workspace_path

### 23.3 下一步仍不扩散

完成 quickstart 后，下一步可以继续回到 eval 语义主线；不需要马上做复杂任务生成器。若要提升用户实验体验，最小后续项是：

1. 提供一个可复制的 task JSON 字段说明
2. 给出“如何修改 old_text/new_text 做自己的实验”的 README 小节
3. 再考虑 `mendcode task init` 这类模板生成命令
