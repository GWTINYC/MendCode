...
### 问题 19：长期记忆如果不可审查，会固化错误事实

状态：第一切片已加边界，后续仍需完善写入审查

现象：

Layered Memory 能减少重复读文件和重复解释项目事实，但如果模型可以静默写入错误事实、敏感内容或过期状态，后续任务会把这些内容当成可信上下文，导致错误被长期放大。

根因：

- 记忆比普通 prompt history 生命周期更长，错误成本更高。
- `memory_search` 是只读能力，适合默认暴露；`memory_write` 是写入长期状态，不能和普通只读查询同等处理。
- trace 失败归因可能有价值，但也可能只是一次环境噪声，不能自动沉淀为长期经验。

处理：

- 记忆统一走 `ToolRegistry` 和 `PermissionPolicy`，由工具风险等级裁剪模型可见工具面。
- TUI 只读自然语言工具面只暴露 `memory_search` 和 `file_summary_read`，不暴露 `memory_write`。
- `memory_write` 和 `file_summary_refresh` 按高风险处理，默认/guided 工具池不暴露。
- `memory_write` 拒绝 exact duplicate 记录，避免同一事实被反复沉淀。
- `trace_analyze` 默认只读，`write_memory=True` 会被拒绝，只返回可审查的 `failure_lesson` 候选。
- `trace_analyze` 限制只能读取 `settings.traces_dir` 内的 trace 文件，避免只读工具探测任意本地路径。
- `MemoryStore.update()` 保留未知或无法解析的 JSONL 行，避免重写时丢失未来 schema 或人工记录。
- `file_summary_read` 复用缓存时必须同时匹配 repo-relative path 和 content hash，避免同内容不同路径串缓存。
- conversation compact log 保留工具 args 和记忆匹配样本，方便复盘模型到底查了什么。

后续约束：

- `memory_write` 必须有确认、去重和敏感信息过滤后才能进入默认工作流。
- 失败 trace 只能先生成候选经验，不能静默写入长期 memory。
- 文件摘要缓存必须按 content hash 校验，过期摘要不能作为当前事实回答。
- memory 相关测试必须同时覆盖工具参数、返回 payload 和可见最终回答，避免只测“答案看起来正确”。

### 问题 20：没有指标入口时，成果数字不可验证

状态：已补基础统计入口，仍需固定 benchmark 数据集

现象：

“工具链路通过率 95%”“高风险命令拦截率 97%”“Token 消耗降低 27%”这类成果如果没有固定测试集、统计口径和历史 baseline，只能作为目标，不能作为项目已验证结果。

根因：

- 现有 unit/scenario/PTY 测试能证明局部行为，但没有统一汇总成产品指标。
- Token 消耗、重复读文件次数、危险命令拦截率需要从 trace、conversation log 和 benchmark case 中统一采集。
- 没有固定 benchmark manifest 时，不同轮次的指标不可比较。

处理：

- 新增 benchmark report 基础模型，统一计算 case 通过率、工具链路通过率、高风险命令拦截率、Token 降低比例和重复读文件次数。
- prompt context 开始记录 observation 数量、memory recall 命中、read_file 次数、重复 read_file 次数和 context 字符量。

后续约束：

- 成果数字必须来自固定 benchmark report，不能手写估计值。
- 修改 prompt、tool schema、memory recall 或 compaction 策略后，需要跑同一组 benchmark 才能比较收益。
- 任何公开成果表述都要区分“目标值”和“已验证值”。
