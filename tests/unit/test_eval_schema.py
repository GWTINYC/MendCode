...
def _summary_payload(**overrides):
    payload = {
        ...
        "results": [
            _result_payload(task_id="task-1"),
            _result_payload(task_id="task-2", status="failed", summary="failed"),
        ],
    }
    payload.update(overrides)
    return payload
...
def test_batch_eval_result_accepts_expected_fields() -> None:
    result = BatchEvalResult.model_validate(_result_payload())
...
def test_batch_eval_result_rejects_running_status() -> None:
    with pytest.raises(ValidationError):
        BatchEvalResult.model_validate(_result_payload(status="running"))


def test_batch_eval_result_rejects_extra_fields() -> None:
    with pytest.raises(ValidationError):
        BatchEvalResult.model_validate(_result_payload(unexpected_field="nope"))


def test_batch_eval_summary_accepts_valid_counts_and_results() -> None:
    summary = BatchEvalSummary.model_validate(_summary_payload())
...
def test_batch_eval_summary_rejects_extra_fields() -> None:
    with pytest.raises(ValidationError):
        BatchEvalSummary.model_validate(
            _summary_payload(unexpected_field="nope")
        )
